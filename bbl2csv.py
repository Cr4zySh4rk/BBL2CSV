...
# ==========================================
# PART 1: Types & Tools (from types.py, tools.py)
# ==========================================
...
# We use 'Context' string forward reference in Callables to avoid definition issues
...
class HeaderDefaults:
    ...
    def inspect(cls, headers: Headers):
        pass # Simplified for standalone script
    @classmethod 
    @property
...
def _tag8_4s16_versioned(data_version: int) -> Decoder:
    return _tag8_4s16_v2 # Force V2 for simplicity in standalone
...
# ==========================================
# PART 8: Main Execution (CLI)
# ==========================================

def main():
    parser_args = argparse.ArgumentParser(description="Standalone BBL to CSV converter")
    parser_args.add_argument("logs", nargs="+", help="Path to .bbl files")
        ...
        try:
            ...
            for idx in target_indexes:
                ...
                with open(out_csv, 'w', newline='') as f:
                    writer = csv.writer(f)
                    # Force 'time' to be first if present (standard convention)
